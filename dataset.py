...
def load_double_image(root_path, file_path, input_height=None, input_width=None, output_height=None, output_width=None,
              crop_height=None, crop_width=None, is_random_crop=True, is_mirror=True, is_gray=False, random_scale=None, align_scale=None):
    ...
    if len(file_path) == 1:
        img = Image.open(join(root_path, file_path[0]))
        ...
        imgR = Image.open(join(root_path, file_path[0]))
        if is_gray is False and imgR.mode != 'RGB':
          imgR = imgR.convert('RGB')
        if is_gray and imgR.mode != 'L':
          imgR = imgR.convert('L')
        
        imgL = Image.open(join(root_path, file_path[1]))
        if is_gray is False and imgL.mode != 'RGB':
          imgL = imgL.convert('RGB')
        if is_gray and imgL.mode != 'L':
          imgL = imgL.convert('L')
    ...
    if random_scale is not None:
        if random.random() < 0.3:
            ...
            imgR = imgR.resize((ww, hh), Image.BICUBIC)
            imgL = imgL.resize((ww, hh), Image.BICUBIC)
    ...
    if align_scale is not None:
      [w, h] = imgR.size
      ww = w // align_scale * align_scale 
      hh = h // align_scale * align_scale
      if w != ww or h != hh:
          imgR = imgR.resize((ww, hh),Image.BICUBIC)
          imgL = imgL.resize((ww, hh),Image.BICUBIC)
    return imgR, imgL
...
class DoubleImageDataset(data.Dataset):
    def __init__(self, image_list, root_path, 
                ...
                is_mirror=True, is_gray=False, normalize=None,  augment=1, random_scale=None, align_scale=None):
        super().__init__()
    ...
    def __getitem__(self, index):          
        ...
        imgR, imgL = load_double_image(self.root_path, self.image_filenames[idx], 
                                  self.input_height, self.input_width, self.output_height, self.output_width,
                                  self.crop_height, self.crop_width, self.is_random_crop, self.is_mirror, self.is_gray, random_scale=self.random_scale, align_scale=self.align_scale)
...
class SingleImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):  
        ...
        idx = index %  self.length  
        img = load_single_image(join(self.root_path, self.image_filenames[idx][0]), 
                                  self.input_height, self.input_width, self.output_height, self.output_width,
                                  self.crop_height, self.crop_width, self.is_random_crop, self.is_mirror, self.is_gray, random_scale=self.random_scale, align_scale=self.align_scale)
